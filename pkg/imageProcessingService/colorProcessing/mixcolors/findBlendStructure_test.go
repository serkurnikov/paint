...
func TestBlendStructure(t *testing.T) {
	...
	blendStructures := BlendStructureAmongFabricColors(mainColorS, MasterColors,
		TypeLABCIEDE2000)
	...
	diff := DifferenceByType(m, r, TypeLABCIEDE2000)

	if diff > DefaultDiff {
		t.Errorf("Incorrect distance got %f, wait < %v,", diff, DefaultDiff)
	} else {
	...
	result := image.NewRGBA(image.Rect(0, 0, WEIGHT, HEIGHT))
...
func BenchmarkSample(b *testing.B) {
	for i := 0; i < b.N; i++ {
		BlendStructureAmongFabricColors("#85739b", MasterColors, TypeLABCIEDE2000)
	}
}
